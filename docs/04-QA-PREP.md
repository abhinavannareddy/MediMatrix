...
> after removal, I fill the day up to a site-wide ceiling, cheapest hours first,
> never letting any hour go above it. That's water filling, the standard
> formulation of peak minimisation.
>
> Then there's a second decision: *which* ceiling. My first instinct was the
> lowest feasible one, because that's the flattest day and the biggest peak cut.
> That turned out to be wrong in the opposite direction to the original bug. If
> you squeeze the ceiling right down, the cheap hours fill up and everything
> after that has to go somewhere expensive. I measured it on a live SE4 day: a
> 469 kW peak cut, but the energy saving fell to 1.9%, and the last zones placed
> were being told to run in the evening peak at a loss.
>
> So I don't pick a ceiling, I search for one. Forty-one candidates from the
> lowest feasible up to the baseline peak, each one priced on both bills, keep
> the best over a year. Placement is O(zones x 24) so the sweep is microseconds.
> Same day, it recovered the energy saving to 11.3% and still cut the peak by
> 486 kW.
>
...
Give five, in order of importance:

> First, authentication and authorization, split into two separate services on
> purpose. auth-service checks who you are and issues a signed token;
> authorization-service checks what your role lets you do. Every API route but
> register, verify and login requires that token, checked by the gateway
> before it proxies anywhere. A "staff" account can read and write; a "viewer"
> account is read-only, and that's enforced server-side, not by hiding a
> button in the browser.
>
> Second, attack surface: only the gateway is exposed. Every other service is
> ClusterIP and cannot be reached from outside the cluster at all.
>
> Third, a default-deny NetworkPolicy. Kubernetes by default lets every pod talk
> to every other pod; I block everything and then open only the conversations
> the system needs. The critical one is that only ingest and auth may reach
> MongoDB - and even between those two, ingest owns the readings collection
> and auth owns the identity collections, so a compromised ingest pod still
> can't touch user data.
>
> Fourth, least-privilege containers: non-root user, no privilege escalation, all
...
>
> Fifth, at the application layer: input validation on every endpoint, HTML
...
>
> Role is self-declared at registration - you pick "staff" or "viewer" on the
> sign-up form yourself, nobody approves it. Fine for a demo, not for a
> hospital. The fix is an approval step, or an invite-only registration flow,
> before an account gets the "staff" role.
>
