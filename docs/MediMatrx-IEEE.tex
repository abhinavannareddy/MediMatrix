...
guaranteeing that patient-critical load is never touched. The system runs as six
microservices and a document database on Kubernetes, reachable from a browser
through a single gateway. Two aspects are treated in detail. The first is the
decomposition, which follows how the components scale rather than how the code
is organised, and places the business logic in exactly one service. The second
is the placement algorithm, which went through three versions: the first raised
the site peak by 938 kW while reporting a reduction of zero, the second fixed
that but cut the energy saving to 1.9\%, and only the third handles both tariff
components at once. Measured over twelve runs on live market data, the deployed
system reduces the daily energy bill by 11.4\% and cuts site peak demand by a
median of 423 kW.
\end{abstract}
...
\caption{Component and deployment architecture. C1--C7 are the logical
...
own something: the meter data, the price cache, or the optimisation algorithm.
Tier 2 services own nothing. The assistant and the forecast service answer
questions and build plans by calling Tier 1, so there is one copy of the
algorithm and one copy of the data in the whole system.
...
C2 & 1 & Metering data management     & \texttt{ingest}    & Node.js 20 / Express & Yes, owns MongoDB \\
...
C7 & --- & Persistence                & \texttt{mongodb}   & MongoDB 7.0 & Yes \\
\bottomrule
\end{tabular}
\end{table*}
...
instead of six times: security headers, rate limiting, request logging, upstream
timeouts. Without it the browser would need six addresses and all six services
would need exposing.

\textbf{C2, Ingest.} Owns the database. Nothing else has credentials for it, and
a NetworkPolicy blocks the connection independently of whether anything else
does.
...
\textbf{C7, MongoDB.} Meter readings are schemaless time-series documents,
written far more than updated and read back through aggregations. A document
store suits that without a migration every time a new meter type appears.
...
Database per Service & Only \texttt{ingest} may reach MongoDB, enforced by credentials and by NetworkPolicy. \\
...
40 resources across 11 manifests, validated against the upstream API schemas
with \texttt{kubeconform --strict}: a Namespace, a ConfigMap and Secret, a
StatefulSet with a \texttt{volumeClaimTemplate}, six Deployments with their
Services, six HorizontalPodAutoscalers, five PodDisruptionBudgets and twelve
NetworkPolicies. Every image is published to Docker Hub so the scheduler can
pull it on any node.
...
\emph{Operational weight.} Forty Kubernetes objects and six images need a build
pipeline and somebody on call. For one hospital that is a bad trade, and it only
becomes reasonable across many sites.
...
\emph{One way in.} Only the gateway has a NodePort. The other five services are
...
twelve policies then open only what is needed. The important one is
\texttt{mongodb-ingress}: only pods labelled \texttt{app: ingest} may open a TCP
connection to the database, so an attacker holding the database password from a
compromised optimiser pod would still be refused by the network. Egress is
...
\begin{itemize}\itemsep2pt
  \item \texttt{services/} --- one directory per microservice, each with its own
        ...
        \texttt{optimizer}, \texttt{assistant}, \texttt{forecast}.
  \item \texttt{k8s/} --- eleven numbered manifests applied in order: namespace,
        configuration and secrets, the MongoDB StatefulSet, the six Deployments
