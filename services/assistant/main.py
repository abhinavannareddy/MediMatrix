...
# --------------------------------------------------------------------------
#  TIME SCOPE
#  The platform holds a rolling 24 hours of meter data and today's (plus
#  tomorrow's) prices. Nothing longer exists anywhere in the system, so a
#  question about a week, a month or a year is not a parsing failure, it is
#  genuinely unanswerable. Saying "I did not follow that" would be a lie: the
#  question was perfectly clear and the data simply is not there.
#
#  Deliberately narrow. "last night" and "annual saving" are both legitimate
#  questions about data we DO have, so neither appears here.
# --------------------------------------------------------------------------
OUT_OF_RANGE = [
    ("year", "a year"), ("month", "a month"), ("week", "a week"),
    ("historical", "historical data"), ("history", "historical data"),
    ("trend", "a trend over time"), ("over time", "a trend over time"),
    ("all time", "all-time records"), ("every day", "day-by-day history"),
    ("past days", "previous days"), ("last few days", "previous days"),
]


FOLLOWUP_OPENERS = (
    "what about", "how about", "and ", "and?", "also", "in the", "in ",
    "for the", "over the", "what if", "then ", "instead", "same for",
    "any others", "others", "more", "why", "ok and", "okay and",
)


def looks_like_followup(text: str) -> bool:
    """Does this read as a continuation of the previous question?"""
    t = text.lower().strip().lstrip("?.,! ")
    return t.startswith(FOLLOWUP_OPENERS)


def detect_out_of_range(text: str) -> str | None:
    """Return a human phrase for the period asked about, or None."""
    t = f" {text.lower()} "
    for needle, phrase in OUT_OF_RANGE:
        if needle in t:
            return phrase
    return None
...
async def answer_explain(area, text, zone):
    ...
    if zone and zone in FLEX_RATIONALE:
        ...
        lines.append(
            ...
            "each receiving hour is capped relative to the zone's typical load so the plan "
...
async def answer_consumption(area, text, zone):
    ...
    if hour is not None:
        ...
        return (
            ...
            f"Its 24-hour total was {kwh(z['totalKwh'])}, against a typical load of "
    ...
    return (
        ...
        f"at {low_h:02d}:00 with {kwh(min(z['hourlyKwh']))}. Typical load "
...
async def answer_out_of_range(area, text, zone, period="that period", carried=None):
    """
    The question was understood and cannot be answered from the data held.
    Say exactly that, say what the window actually is, and offer the nearest
    question that does have an answer. A tool that explains its own limits is
    ...
    topic = {
        ...
        "carbon":      "carbon",
        "peak":        "peak demand",
        "anomaly":     "equipment faults",
    }.get(carried or "", None)

    lead = f"Still on {topic}: I" if topic else "I"

    return (
        f"{lead} cannot answer that, because I do not hold {period}.\n\n"
        "**What this platform actually stores:** a rolling **24 hours** of "
        "meter readings, today's hourly prices, and tomorrow's once the "
        "market publishes them in the early afternoon. Nothing longer is "
        "kept anywhere in the system, so there is no year to search.\n\n"
        "That is a storage decision rather than a limit of mine. Keeping a "
        "daily summary per zone would make questions like yours answerable "
        "after a few weeks of running.\n\n"
        "What I can tell you right now: today's cheapest and most expensive "
        "hour, what any zone used in the last 24 hours, today's savings and "
        "peak reduction, and tomorrow's plan.",
        [], {"outOfRange": period})
...
class ChatContext(BaseModel):
    """
    What the previous turn was about, echoed back by the browser.

    Conversation state deliberately does NOT live in this service. The
    assistant is stateless so that any replica can answer any request, which
    is the whole reason it can be scaled horizontally; a session dictionary
    here would mean replica A knowing nothing of what replica B just said,
    and would need sticky routing or a shared cache to fix. Handing the
    context to the client and taking it back is simpler, survives a pod
    restart, and keeps the scaling property intact.
    """
    lastIntent: str | None = Field(default=None, max_length=32)
    lastZone: str | None = Field(default=None, max_length=32)
...
async def chat(req: ChatRequest):
    ...
    carried = req.context.lastIntent

    # A follow-up often carries no keywords of its own ("and in the whole
    # year?", "what about the laundry?"). Inheriting the previous intent is
    # what makes the difference between a conversation and a series of
    # unrelated questions.
    #
    # Only when the phrasing actually looks like a follow-up, though. An
    # unrecognised question is not automatically a continuation: carrying
    # the intent unconditionally would answer "banana helicopter" with a
    # confident paragraph about electricity prices, which is worse than
    # admitting the question was not understood.
    if intent == "unknown" and carried and looks_like_followup(req.message):
        intent, score = carried, 1
        _stats["carried"] = _stats.get("carried", 0) + 1

    if zone is None and req.context.lastZone and intent == "consumption":
        zone = req.context.lastZone
    ...
    # Checked before dispatch: a question about a year is clear, not
    # confusing, and deserves a straight answer rather than a guess about
    # today dressed up as a response.
    period = detect_out_of_range(req.message)

    # The subject of the conversation, kept separately from the intent that
    # actually gets dispatched. "cheapest day of the year" is about prices
    # even though it is answered by the out-of-range handler, and the next
    # follow-up should still know we are talking about prices.
    topic = intent if intent != "unknown" else carried
    ...
    try:
        if period:
            intent = "out-of-range"
            reply, sources, data = await answer_out_of_range(
                req.area, req.message, zone, period, topic)
        else:
    ...
    return {"pod": POD, "intent": intent, "confidence": score, "zone": zone,
            ...
            # Handed back for the next turn. See ChatContext for why this
            # lives in the client rather than in the service.
            "context": {"lastIntent": topic if topic != "unknown" else None,
                        "lastZone": zone}}
