...
# Grid demand charge (effektavgift), SEK per kW of the month's highest hour.
# Swedish network operators bill this on top of energy, and it is real money:
# the bill is set by one hour out of roughly 730.
#
# The figure below is the BASE monthly rate for a high-voltage business
# subscription. Two real 2025/26 tariffs for comparison:
#
#   Vattenfall Eldistribution   11-34 kr/kW/month base, plus a hogbelastning
#                               surcharge of 21-86 kr/kW/month charged only in
#                               Jan, Feb, Mar, Nov and Dec
#   Skanska Energi              265 kr/kW/year flat, about 22 kr/kW/month
#
# 34.0 is the top of Vattenfall's base band and is used deliberately as a
# conservative choice: it ignores the winter surcharge entirely, so the
# reported demand saving understates what a real customer on that tariff
# would see for five months of the year. An earlier version used 68.0, which
# is above the base range of both operators and overstated this component.
DEMAND_CHARGE_SEK_PER_KW_MONTH = 34.0
...
def optimise(summary: dict, prices: dict, flex_overrides: dict | None = None) -> dict:
    ...
    # ---- 3b. Choose the ceiling that is worth the most money --------------
    #
    # The obvious move is to pick the LOWEST feasible ceiling, because that
    # gives the flattest day and the biggest peak reduction. That is what an
    # earlier version did, and it was subtly wrong in the opposite direction
    # to the bug before it.
    #
    # Squeezing the ceiling right down means the cheap hours fill up, and
    # everything after that has to go somewhere more expensive. The peak
    # looks magnificent and the energy bill barely moves. Worse, the zones
    # processed last get pushed into genuinely expensive hours, so their
    # individual recommendations end up LOSING money even though the plan as
    # a whole still wins on the demand charge. Telling an estates manager to
    # run the laundry at 18:00 is not advice they will take twice.
    #
    # So we do not guess. We sweep candidate ceilings from the lowest
    # feasible one up to the baseline peak, price each resulting plan on BOTH
    # bills, and keep the best. Placement is O(zones x 24) and the sweep is
    # ~40 candidates, so this is microseconds, and it removes the need to
    # hand-tune a trade-off that changes with every price curve.
    floor_ceiling = lowest_feasible_ceiling(
        after_removal_total, placement_hours, total_movable, site_peak
    )

    def place(ceiling: float):
        """Fill up to `ceiling`, cheapest hours first. Returns (plans, totals)."""
        trial = {zid: list(p) for zid, p in plans.items()}
        ...
        for z in zones:
            ...
            # laundry has a fixed number of machines; 1.6x its normal draw
            # is a generous but finite limit.
            ...
            # If the zone's own power limit stopped us placing everything,
            # the remainder goes into whichever hour is currently LOWEST, so
            # even the last resort flattens the day instead of spiking it.
            if remaining > 0.01:
            ...
            windows[zone_id] = receiving

        return trial, site_now, windows

    def value_of(site_totals: list[float], trial: dict) -> float:
        """Annual value of a plan, counting BOTH bills."""
        cost = sum(
            trial[z["zoneId"]][h] * hourly_price[h]
            for z in zones for h in range(24)
        )
        peak_cut = site_peak - max(site_totals)
        return (baseline_cost - cost) * 365 + peak_cut * DEMAND_CHARGE_SEK_PER_KW_MONTH * 12

    best = None
    span = max(site_peak - floor_ceiling, 0.0)
    # 41 candidates: the flattest plan, the baseline peak, and 39 in between.
    for i in range(41):
        cand = floor_ceiling + span * (i / 40.0)
        trial, totals, windows = place(cand)
        v = value_of(totals, trial)
        if best is None or v > best[0]:
            best = (v, cand, trial, totals, windows)

    _, ceiling, plans, site_now, windows = best
    ...
    for z in zones:
        ...
        # A move can still cost money on energy while paying for itself on
        # the demand charge, and when that happens the recommendation has to
        # say so in plain words. "Saves -31 kr" is not a sentence anybody
        # should read on a dashboard.
        if saving >= 0:
            text = (
            ...
            text = (
                f"Move {grouped(movable)} kWh of {z['name']} load into "
                f"{window_label}. This costs {grouped(-saving)} kr more in "
                f"energy today, but it is part of holding the site peak down, "
                f"which is worth far more on the grid demand charge. Do this "
                f"one only as part of the whole plan, not on its own."
            )
            priority = "low"
